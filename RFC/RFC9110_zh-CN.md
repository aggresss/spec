...
“保护空间”是由被访问服务器的来源(参见4.3.1节)和 realm 值(如果存在的话)定义的。这些领域允许将服务器上受保护的资源划分为一组保护空间，每个空间都具有自己的身份验证方案和/或授权数据库。realm 值是一个字符串，通常由源服务器分配，它可以具有特定于身份验证方案的额外语义。请注意，同 一个 auth-scheme 的响应可能有多个不同域的挑战。
