...
无论初始请求的 method 是何种 method，只要 CANCEL 匹配了一个现存的事务，这个 UAS 应该自己应答这个 CANCEL，并且返回一个 200(OK)响应。这个响应消息是通过以下处理流程来创建的，具体创建流程查阅，这里需要注意，对于 CANCEL来说，这个响应中的这个 To 标签和初始请求中的响应中的 To 标签应该相同。这个CANCEL 的回复响应被传递到这个服务器的事务处理进行传输。
