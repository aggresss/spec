...
- 树由节点组成。如果节点没有子节点，则为叶子节点；否则，为父节点。
- 树中的所有父节点都恰好有两个子节点，一个左子节点和一个右子节点。
- 如果节点没有父节点，则为树的根；如果节点同时具有父节点和子节点，则为中间节点。
- 节点的后代是该节点的子节点，以及其子节点的后代。如果节点是树根的后代，或者该节点本身是树的根，则我们说树包含节点。
- 如果节点共享同一个父节点，则它们彼此为兄弟节点。

- 树的子树是由任意节点（子树的头）及其后代给出的树。
- 树或子树的大小是它包含的叶节点数。
- 对于给定的父节点，其左子树是以其左子节点为头的子树，其右子树是以其右子节点为头的子树。

每棵树都是完美二叉树，即具有 $2^{d}$ 个叶子节点的树是具有相同深度 d 的完全平衡二叉树。此结构对于给定的深度 d 而言是唯一的。

实现可能用多种方式在内存中表示棘轮树。左平衡二叉树（包括此处使用的完整树）的一个方便属性是它们可以表示为节点数组，节点关系基于数组中节点的索引计算。也可以使用基于链接节点对象的更传统的表示。附录 C 和 D 提供了有关如何在这些表示中实现 MLS 所需的树操作的一些详细信息。MLS 对实现的棘轮树的内部表示没有任何要求。实现可以使用任何树表示和相关算法，只要它们能生成正确的协议消息即可。

#### 4.1.1.  Ratchet Tree Nodes

棘轮树中的每个叶节点都有一个索引（或叶索引），从左侧的 0 开始到右侧的 $2^{d} - 1$（对于具有 $2^{d}$ 个叶子的树）。具有 $2^{d}$ 个叶子的树有 $2^{d+1} - 1$ 个节点，包括父节点。

棘轮树中的每个节点要么是空白的（不包含任何值），要么持有带有一些相关数据的 HPKE 公钥：

- 公钥（用于正在使用的 HPKE 方案；参见第 5.1 节）
- 凭证（仅适用于叶节点；参见第 5.3 节）
- 未合并叶子节点的有序列表（参见4.2 节）
- 关于节点父节点的某些信息的哈希值，截至上次更改节点时（参见第 7.9 节）

如第 4.2 节所述，不同的成员知道树中节点的公钥对应的私钥集的不同子集。只有叶节点的后代成员才知道父节点对应的私钥。只有叶节点的成员才知道叶节点对应的私钥。如果叶节点的成员不知道祖先节点对应的私钥，则叶节点相对于其祖先节点之一取消合并。

每个节点，无论该节点是空白的还是已填充的，都有一个对应的哈希值，该哈希值总结了该节点下方子树的内容。计算这些哈希值的规则在第 7.8 节中描述。

节点的解析是非空白节点的有序列表，这些非空白节点共同覆盖节点的所有非空白后代。根的解析包含一组密钥，这些密钥对于加密组中的每个节点都是必需的。节点的解析实际上是节点下方最近的非空白节点的深度优先、左优先枚举：

- 非空白节点的解析包括节点本身，后面是未合并叶子的列表（如果有）
- 空白叶节点的解析度为空列表
- 空白中间节点的解析是将其左子节点的解析与其右子节点的解析按顺序连接起来的结果

例如，考虑以下子树，其中 _ 字符代表空白节点，未合并的叶子用方括号表示：
...
根的直接路径是空列表。任何其他节点的直接路径都是该节点的父节点与父节点的直接路径的连接

节点的共路径是该节点的兄弟节点与其直接路径中所有节点的兄弟节点列表连接起来，不包括根节点

叶节点 L 的过滤直接路径是该节点的直接路径，其中删除了任何在 L 的共路径上的子节点具有空解析度的节点（请记住，共路径子节点的任何未合并的叶子都计入其解析度）。删除的节点不需要自己的密钥对，因为加密到节点的密钥对相当于加密到其非共路径子节点

例如，考虑以下树（其中空白节点用 表示 _，但也分配有标签以供参考）：
...
我们通常假设每个参与者都维护着该组棘轮树的完整公共状态和最新视图，包括所有节点的公钥和与叶节点相关联的凭据。

MLS 组中的参与者不知道与树中每个节点关联的私钥。相反，每个成员被分配到树的一个叶子节点，这决定了它知道的私钥子集。存储在该叶子上的凭证是由成员提供的。

具体来说，MLS 以这样的方式维护成员对树的视图，从而保持树的不变：

- **仅当节点的子树包含该成员的叶子节点时，树中节点的私钥才为组成员所知**

换句话说，如果一个节点不为空，那么它就拥有一个公钥。只有占据该节点下方叶子节点的成员才知道相应的私钥。

反过来的推论是不正确的：成员可能不知道他们上方的中间节点的私钥。这样的成员在中间节点有一个未合并的叶子。加密到中间节点需要加密到节点的公钥，以及它下面所有未合并叶子的公钥。叶子在第一次添加时相对于其所有祖先都是未合并的，因为添加叶子的过程并不能让它访问树中它上方所有节点的私钥。叶子在收到节点的私钥时被“合并”，如 7.4 节所述。

例如，考虑一个四成员组（A，B，C，D），其中右边两个成员上方的节点为空白。（如果 A 与 B、C 和 D 创建一个组，情况将会如此。）那么树的公共状态和每个参与者持有的树的私钥视图将如下所示，其中 _ 表示空白节点，? 表示未知私钥，pk(X) 表示与私钥 X 对应的公钥：
...
每个 MLS 会话使用一个密码套件，该套件指定在组密钥计算中使用的以下原语：
...
MLS 使用 HPKE 进行公钥加密 [RFC9180]。DeriveKeyPair 与密码套件的 KEM 关联的函数将八位字节串映射到 HPKE 密钥对。与 HPKE 一样，MLS 假设 AEAD 算法从 AEAD 加密中生成单个密文输出（与 [RFC5116]一致），而不是单独的密文和标签

密码套件使用 CipherSuite Type 表示，定义在第 17.1 节中。

#### 5.1.1. Public Keys

HPKE 公钥是采用底层协议定义格式的不透明值（有关更多信息，请参阅 [RFC9180] 第 4 节）

```
  opaque HPKEPublicKey<V>;
```

签名公钥同样以密码套件的签名方案定义的格式表示为不透明值。

```
  opaque SignaturePublicKey<V>;
```

对于使用 Edwards曲线数字签名算法（EdDSA）签名方案（Ed25519或Ed448）的密码套件，公钥采用 [RFC8032] 中指定的格式。

对于使用椭圆曲线数字签名算法（ECDSA）和 NIST 曲线（P-256、P-384或P-521）的密码套件，公钥表示为编码的未压缩点表示结构，如 [RFC8446] 中所定义。

#### 5.1.2. Signing

群组密码套件中指定的签名算法是群组内消息签名的强制算法。它必须与树叶中凭证中指定的签名算法相同（包括用于添加新成员的 KeyPackages 中的叶节点信息）

本文档中使用的签名按照 [RFC8446] 中的规定进行编码。具体来说，ECDSA 签名是 DER 编码，而 EdDSA 签名定义为 R 和 S 的拼接， [RFC8032] 中所述。¶

为了消除 MLS 中使用的不同签名的歧义，每个签名值都以标签为前缀，如下所示：
...
函数 Signature.Sign 和 Signature 由算法定义。如果 MLS 扩展需要组成员签名，则它们应重用 SignWithLabel 构造，使用不同的标签。为了避免这些标签发生冲突，在第 17.6 节 IANA 注册表 中定义。

#### 5.1.3. Public Key Encryption

与签名一样，MLS 在加密操作中包含标签和上下文，以避免为不同目的生成的密文之间产生混淆。包括此标签和上下文的加密和解密如下：
...
函数 SealBase 和 OpenBase 在 [RFC9180] 的第 6.1 节中定义（以“Base”作为模式），使用由该组的密码套件指定的 HPKE 算法。如果 MLS 扩展需要 HPKE 加密操作，则它们应使用不同的标签重用 EncryptWithLabel 构造。为了避免这些标签冲突，第 17.7 节中定义了 IANA 注册表 。
