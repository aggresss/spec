...
> Origin [https://datatracker.ietf.org/doc/html/draft-gg-udt-03](https://datatracker.ietf.org/doc/html/draft-gg-udt-03)
...
Following are the 32-bit time stamp when the packet is sent and the destination socket ID. The time stamp is a relative value starting from the time when the connection is set up. The time stamp information is not required by UDT or its native control algorithm. It is included only in case that a user defined control algorithm may require the information (See Section 6).
